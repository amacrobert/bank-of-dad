...
func TestApplyInterest(t *testing.T) {
	...
	// Apply interest: 20000 * 1000 / 12 / 10000 = 166.67 → 167 cents
	err = is.ApplyInterest(child.ID, parent.ID, 1000)
	require.NoError(t, err)
...
func TestApplyInterest_UpdatesLastInterestAt(t *testing.T) {
	...
	// Apply interest
	err = is.ApplyInterest(child.ID, parent.ID, 500)
	require.NoError(t, err)
...
func TestApplyInterest_CalculationExamples(t *testing.T) {
	tests := []struct {
		...
		{"$100 at 5%", 10000, 500, 42},         // 10000 * 500 / 12 / 10000 = 41.67 → 42
		{"$100 at 10%", 10000, 1000, 83},        // 10000 * 1000 / 12 / 10000 = 83.33 → 83
		{"$1000 at 5%", 100000, 500, 417},       // 100000 * 500 / 12 / 10000 = 416.67 → 417
		{"$50 at 12%", 5000, 1200, 50},          // 5000 * 1200 / 12 / 10000 = 50
		{"$1 at 5%", 100, 500, 0},               // 100 * 500 / 12 / 10000 = 0.42 → 0 (skip)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = is.ApplyInterest(child.ID, parent.ID, tt.rateBps)
...
func TestApplyInterest_ZeroRate(t *testing.T) {
	...
	err = is.ApplyInterest(child.ID, parent.ID, 0)
	assert.Error(t, err)
}
