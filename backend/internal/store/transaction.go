...
func (s *TransactionStore) GetByID(id int64) (*Transaction, error) {
	...
	).Scan(&t.ID, &t.ChildID, &t.ParentID, &t.AmountCents, &t.TransactionType, &note, &createdAt)
...
func (s *TransactionStore) ListByChild(childID int64) ([]Transaction, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.ChildID, &t.ParentID, &t.AmountCents, &t.TransactionType, &note, &createdAt); err != nil {
